...
export default function FinancialAffairs({
  ...
  // Date boundary calculation for filters
  ...
  const filteredTransactions = transactions.filter((tx) => {
    ...
    if (periodFilter === 'week' && (tx.date < oneWeekAgoStr || tx.date > todayStr)) return false;
    if (periodFilter === 'month' && !tx.date.startsWith(currentMonthStr)) return false;

    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase().trim();
      const catLabel = getCategoryLabel(tx.category).toLowerCase();
      const notes = (tx.notes || '').toLowerCase();
      const amountStr = tx.amount.toString();
      if (!catLabel.includes(q) && !notes.includes(q) && !amountStr.includes(q)) {
        return false;
      }
    }
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-100 space-y-3">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <h3 className="text-xs font-bold text-[#024B83] font-arabic uppercase tracking-wider flex items-center gap-1.5">
              <ClipboardList className="w-4 h-4 text-[#E5941A]" />
            ...
            <div className="relative min-w-[200px] max-w-xs">
              <input
                ...
                placeholder={isRtl ? 'بحث بالملاحظات أو البند أو المبلغ...' : 'Search note, account or amount...'}
                className="w-full py-1.5 px-3 bg-white border border-slate-200 rounded-lg text-slate-800 text-xs font-medium focus:outline-hidden focus:ring-1 focus:ring-[#024B83] font-arabic"
              />
              {searchQuery && (
                <button
                  type="button"
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-200/60">
            {/* Period Filter Tabs */}
            <div className="flex flex-wrap items-center gap-1.5 text-xs font-arabic">
              <button
                onClick={() => setPeriodFilter('all')}
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  periodFilter === 'all' ? 'bg-[#024B83] text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
                {t.filterAll}
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  periodFilter === 'today' ? 'bg-[#024B83] text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
                {t.filterToday}
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  periodFilter === 'week' ? 'bg-[#024B83] text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
                {t.filterThisWeek}
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  periodFilter === 'month' ? 'bg-[#024B83] text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
                {t.filterThisMonth}
              </button>
            ...
            {/* Type Filter Buttons */}
            <div className="flex gap-1.5 text-xs font-arabic">
              <button
                onClick={() => setFilterType('all')}
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  filterType === 'all' ? 'bg-slate-800 text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
                {isRtl ? 'الكل' : 'All'}
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  filterType === 'income' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-white text-emerald-700 hover:bg-emerald-50 border border-emerald-200'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
                  filterType === 'expense' ? 'bg-red-600 text-white shadow-xs' : 'bg-white text-red-700 hover:bg-red-50 border border-red-200'
                }`}
              >
